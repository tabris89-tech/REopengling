...
from opengling.core.models import ExportFormat, ProcessingConfig, parse_time_to_seconds
...
@app.command()
def process(
    input_path: Optional[Path] = typer.Argument(None, help="Path to input video file"),
    ...
    # URL download
    url: Optional[str] = typer.Option(None, "--url", help="Download video from URL, then process it"),
    cookies: Optional[str] = typer.Option(None, "--cookies", help="Browser for cookies (chrome, firefox, edge)"),
    keep_source: bool = typer.Option(False, "--keep-source", help="Keep downloaded file after processing"),
    ...
    # Parse time range (used by both --url and local file trimming)
    start_time = None
    end_time = None
    if start:
        try:
    ...
    # Handle URL download
    if url:
        from opengling.core.url_downloader import download_url, inspect_url

        console.print(f"[cyan]🔍 Inspecting URL:[/cyan] {url}")
        info = inspect_url(url)

        if info.type == "unsupported" or info.type == "error":
            console.print(f"[red]Error: {info.error or 'URL not supported'}[/red]")
            raise typer.Exit(1)
        ...
        if info.type == "playlist":
            if len(info.videos) == 1:
                ...
                console.print(f"[yellow]📋 Playlist: {info.playlist_title} ({len(info.videos)} videos)[/yellow]")
                console.print("[yellow]Use 'opengling download' for playlists[/yellow]")
                console.print("[yellow]Processing only the first video...[/yellow]")
                video = info.videos[0]
        else:
            video = info.videos[0]
        ...
        filesize_str = ""
        if video.filesize:
            filesize_str = f", {video.filesize / (1024*1024):.0f} MB"
        console.print(f"[green]  Found:[/green] {video.title} ({_format_duration(video.duration)}{filesize_str})")

        # Download with progress (uses yt-dlp --download-sections if start/end given)
        download_dir = Path(tempfile.mkdtemp(prefix="opengling_dl_"))
        with console.status("[cyan]Downloading...") as status:
            def dl_progress(percent: float, text: str):
                status.update(f"[cyan]{text} ({percent*100:.0f}%)")
        ...
        console.print("[red]Error: Specify a file path or --url[/red]")
        raise typer.Exit(1)
    elif not input_path.exists():
        str_path = str(input_path)
        if str_path.startswith(("http://", "https://", "ftp://")):
            console.print("[yellow]Hint: Use --url to download from URLs[/yellow]")
    ...
    # Cleanup: if we downloaded from URL and user didn't --keep-source
    if url and not keep_source and download_dir and download_dir.exists():
        import shutil
        shutil.rmtree(download_dir, ignore_errors=True)
        console.print("[dim]Temp download cleaned up[/dim]")
...
def serve(
    ...
    console.print("[cyan]Starting OpenGling Web Server...[/cyan]")
...
def _select_from_playlist(videos: list) -> Optional[dict]:
    """Show interactive playlist selection, return selected video."""
    table = Table(title="📋 Playlist", show_header=True, header_style="bold cyan")
    table.add_column("#", style="cyan", width=4)
    table.add_column("Title", style="white")
    table.add_column("Duration", style="green")
    for i, v in enumerate(videos, 1):
        table.add_row(str(i), v.title, _format_duration(v.duration))
    console.print(table)

    while True:
        choice = typer.prompt("Выберите видео (1-{n}), 'all' или 'q' для выхода".format(n=len(videos)))
        if choice.lower() == "q":
            return None
        if choice.lower() == "all":
            return "__all__"
...
def inspect(
    ...
    """
    Inspect a URL and show information about videos found.

    Examples:

        opengling inspect https://youtube.com/watch?v=...

        opengling inspect https://youtube.com/playlist?list=...
    """
    from opengling.core.url_downloader import inspect_url

    console.print(f"[cyan]🔍 Inspecting:[/cyan] {url}")
    info = inspect_url(url)
    ...
    if info.type == "playlist" and len(info.videos) > 1:
        console.print(f"\n[bold]📋 Playlist:[/bold] {info.playlist_title}")
        console.print(f"   Videos: {len(info.videos)}")
        console.print()
        ...
        for i, v in enumerate(info.videos, 1):
            size = f"{v.filesize/(1024*1024):.0f} MB" if v.filesize else "—"
            table.add_row(str(i), v.title, _format_duration(v.duration), size)
        console.print(table)
    else:
        v = info.videos[0]
        size = f"{v.filesize/(1024*1024):.0f} MB" if v.filesize else "—"
        res = f" ({v.resolution})" if v.resolution else ""
        console.print()
        console.print(Panel(
            f"[bold cyan]🎬 {v.title}[/bold cyan]\n"
            f"   Duration: {_format_duration(v.duration)}\n"
            f"   Size: {size}{res}\n"
            f"   URL: {v.webpage_url or v.url}",
            title="Video Info",
        ))
...
def download(
    ...
    output_dir: Optional[Path] = typer.Option(None, "--output-dir", "-o", help="Directory to save video"),
    cookies: Optional[str] = typer.Option(None, "--cookies", help="Browser for cookies (chrome, firefox, edge)"),
    format: str = typer.Option("mp4", "--format", "-f", help="Download format: mp4, best"),
    download_playlist: str = typer.Option("ask", "--playlist", "-p", help="Playlist mode: ask, all, first"),
    start: Optional[str] = typer.Option(None, "--start", help="Start time (HH:MM:SS or MM:SS)"),
    end: Optional[str] = typer.Option(None, "--end", help="End time (HH:MM:SS or MM:SS)"),
    process_after: bool = typer.Option(False, "--process", help="Process video after download"),
    keep_source: bool = typer.Option(False, "--keep-source", help="Keep source file after processing"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Verbose output"),
):
    """
    ...
    Supports:
      - YouTube, VK, RuTube, Vimeo, Twitch, and 1700+ sites (via yt-dlp)
      - Direct links to MP4, WebM, AVI, MOV files (via HTTP streaming)
      - Time range: use --start/--end to download only a portion

    Examples:

        opengling download https://youtube.com/watch?v=...

        opengling download https://youtube.com/playlist?list=... -o ./videos

        opengling download https://example.com/video.mp4

        opengling download https://... --start 00:10:00 --end 00:20:00

        opengling download https://... --cookies chrome --process
    """
    logging.basicConfig(
        level=logging.DEBUG if verbose else logging.WARNING,
        format="%(message)s",
    ...
    # Parse time range
    import opengling.core.models as models
    start_time = None
    end_time = None
    if start:
        try:
            start_time = models.parse_time_to_seconds(start)
        except ValueError as e:
            console.print(f"[red]Error: {e}[/red]")
            raise typer.Exit(1)
    if end:
        try:
            end_time = models.parse_time_to_seconds(end)
        except ValueError as e:
            console.print(f"[red]Error: {e}[/red]")
            raise typer.Exit(1)
    ...
    # Handle playlists
    if info.type == "playlist" and len(info.videos) > 1:
        if download_playlist == "first":
            videos_to_dl = [info.videos[0]]
        elif download_playlist == "all":
            videos_to_dl = info.videos
        else:  # ask
            console.print(f"\n[bold]📋 Playlist:[/bold] {info.playlist_title} ({len(info.videos)} videos)")
            choice = _select_from_playlist(info.videos)
            if choice is None:
        ...
        with Progress(
            ...
            task = progress.add_task("[cyan]Downloading...", total=len(videos_to_dl))
            for v in videos_to_dl:
                progress.update(task, description=f"[cyan]Downloading: {v.title[:40]}...")
                dl_dir = Path(tempfile.mkdtemp(prefix="opengling_dl_"))
                def _make_cb(task_id=task, p=progress):
                    def cb(percent: float, text: str):
                        p.update(task_id, completed=int(percent * 100) if percent < 1.0 else 1)
                    return cb
                downloaded = download_url(
                    v.url,
                    output_dir=dl_dir,
                ...
                # Move to output dir
                dest = output_dir / downloaded.name
                counter = 1
                while dest.exists():
        ...
        console.print(f"\n[green]✓[/green] Downloaded {len(results)} files to [bold]{output_dir}[/bold]")

        # Optionally process
        if process_after and results:
            for path in results:
                console.print(f"\n[cyan]Processing: {path.name}[/cyan]")
                # Call process logic via Typer
                from opengling.core.processor import VideoProcessor
                processor = VideoProcessor(ProcessingConfig())
                processor.process(path)
                proc_output = output_dir / f"{path.stem}_edited{path.suffix}"
                console.print(f"[green]✓[/green] Output: {proc_output}")
                if not keep_source:
                    path.unlink(missing_ok=True)
    else:
        # Single video
        v = info.videos[0]
        filesize_str = f" ({v.filesize / (1024*1024):.0f} MB)" if v.filesize else ""
        console.print(f"[green]  Found:[/green] {v.title} ({_format_duration(v.duration)}{filesize_str})")
        ...
        console.print(f"[green]✓[/green] Downloaded: [bold]{downloaded}[/bold]")

        # Optionally process
        if process_after:
            console.print(f"\n[cyan]Processing: {downloaded.name}[/cyan]")
            from opengling.core.processor import VideoProcessor
            processor = VideoProcessor(ProcessingConfig())
            result = processor.process(downloaded)
            if result.output_path:
                console.print(f"[green]✓[/green] Output: [bold]{result.output_path}[/bold]")
            if not keep_source:
                downloaded.unlink(missing_ok=True)
                console.print("[dim]Source file removed[/dim]")
